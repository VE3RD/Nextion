...
if [ -z "$2" ]; then
    ...
    if [ "$1" = 2 ]; then 
	sudo sed -i '/^\[/h;G;/Network/s/\(Startup=\).*/\1'"$2"'/m;P;d' /etc/nxdngateway
    fi
 
    if [ "$1" = 3 ]; then 
	sudo sed -i '/^\[/h;G;/DMR Network/s/\(Address=\).*/\1'"$2"'/m;P;d' /etc/ysf2dmr
    fi
    ...
    if [ "$1" = 6 ]; then 
#	sudo sed -i '/^\[/h;G;/NXDN Network/s/\(Id=\).*/\1'"$2"'/m;P;d' /etc/ysf2nxdn
