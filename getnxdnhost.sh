#!/bin/bash
############################################################
#  Lookup NXDN Host /usr/local/etc/NXDNHosts.txt 		   #
...
line1=$(sudo sed -n '/'"$1"'/p' /usr/local/etc/NXDNHosts.txt | sed 's/,*\([^"]\)/\1/g;s/\([^"]\),*/\1/g' | sed -E "s/[[:space:]]+/|/g" | sed '1q;d')
line2=$(sudo sed -n '/'"$1"'/p' /usr/local/etc/NXDNHosts.txt | sed -E "s/[[:space:]]+/|/g" | sed '2q;d')
